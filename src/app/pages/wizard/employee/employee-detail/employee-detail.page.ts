...
export class EmployeeDetailWizardPage implements OnInit, OnDestroy {
  ...
  async saveEmployee() {
    ...
    if (!this.isEdit) {
      this.subscription.add(this.employeeService.createEmployee(employee).subscribe((newEmployee: Employee) => {
        if (newEmployee) {

          const timeTable = this.transformTimeTableModel(newEmployee);
          if (newEmployee?.uuid === this.employee?.uuid) {
            timeTable.uuid = this.employee.timetable.uuid;
          }
          if (this.imageChanged) {
            forkJoin([this.uploadService.sendUploadEmployeeImage(this.imageFile, 'employee', newEmployee.uuid), this.timeTableService.createEmployeeTimeTable(timeTable)]).subscribe(res => {
              if (res) {
                this.cancel();
              }
            });
    ...
    } else if (this.employee.isOwner && this.isEmployee && this.serviceSelected.length === 0) {
      const alert = await this.alertCtrl.create({
        cssClass: 'my-custom-class',
        header: 'Debe asignar al menos un servicio para poder aparecer en la agenda',
        buttons: [
          {
            text: 'Aceptar',
            id: 'confirm-button',
  ...
  goToHourDayDetail(day: string) {
    ...
    const navigationExtras: NavigationExtras = {
      state: { day, scheduleDay, timetableId, isCommerce: false, newEmployee: !this.employee ? true : false },
  ...
  gotoHoliday() {
    ...
    this.navCtrl.navigateForward(['tabs/profile/commerce-info/admin-employee/employee-detail/holiday'], navigationExtras);
  }
  ...
  async presentServiceModal() {
    const modal = await this.modalController.create({
      ...
      componentProps: {
        ...
        serviceCollectionFiltered: this.serviceCollection, serviceCollection: this.serviceCollection, isConfigScreen: true
      }
    });
    await modal.present();
  ...
  cancel() {
    this.navCtrl.navigateBack(['wizard/employee'], { replaceUrl: true });
  }
  ...
  goToSelectRole() {
    const roleSelected = this.employeeForm.get('role').value;
    this.navCtrl.navigateForward(['select-role'], { relativeTo: this.activatedRoute, state: { roleSelected, isWizard: true } });
  }
  ngOnDestroy(): void {
